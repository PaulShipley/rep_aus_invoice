...
function print_sales_quotations()
{
	...
	include_once(find_custom_file("/reporting/includes/pdf_report.inc"));  // PS
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		while ($myrow2=db_fetch($result))
		{
			...
			$c=0;  // PS
			$rep->TextCol($c++, $c,	$myrow2['stk_code'], -2);  // PS
			$oldrow = $rep->row;
			$rep->TextColLines($c++, $c, $myrow2['description'], -2);  // PS
			$newrow = $rep->row;
			$rep->row = $oldrow;
			if ($Net != 0.0 || !is_service($myrow2['mb_flag']) || !$SysPrefs->no_zero_lines_amount())
			{
				$rep->TextCol($c++, $c,	$DisplayQty, -2);  // PS
				$rep->TextCol($c++, $c,	$myrow2['units'], -2);  // PS
				$rep->TextCol($c++, $c,	$DisplayDiscount, -2);  // PS
				$rep->TextCol($c++, $c,	$DisplayPrice, -2);  // PS
				$rep->TextCol($c++, $c,	'', -2);  // PS GST not available on Sales Quotations
				$rep->TextCol($c++, $c,	$DisplayNet, -2);  // PS
			}
			$rep->row = $newrow;
		...
		$rep->aligns[3] = 'left';  // PS
		$rep->TextCol(3, 6, _("Sub-total"), -2);
		$rep->TextCol(7, 8,	$DisplaySubTot, -2);  // PS
		$rep->NewLine();
		if ($myrow['freight_cost'] != 0.0)
		{
			$DisplayFreight = number_format2($myrow["freight_cost"],$dec);
			$rep->TextCol(3, 6, _("Shipping"), -2);
			$rep->TextCol(7, 8,	$DisplayFreight, -2);  // PS
			$rep->NewLine();
		}	
		$DisplayTotal = number_format2($myrow["freight_cost"] + $SubTotal, $dec);
		if ($myrow['tax_included'] == 0) {
			$rep->TextCol(3, 6, _("TOTAL ORDER EX GST"), - 2);  // PS
			...
			if ($myrow['tax_included'])
			{
				if ($SysPrefs->alternative_tax_include_on_docs() == 1)
				{
					if ($first)
					{
						...
						$rep->TextCol(7, 8,	number_format2($tax_item['net_amount'], $dec), -2);  // PS
						$rep->NewLine();
					}
					$rep->TextCol(3, 6, $tax_type_name, -2);
					$rep->TextCol(7, 8,	$DisplayTax, -2);  // PS
					$first = false;
				}
				else
		...
		$DisplayTotal = number_format2($myrow["freight_cost"] + $SubTotal, $dec);
		$rep->Font('bold');
		$rep->TextCol(3, 7, $DisplayTax ? _("TOTAL ORDER INCLUDES GST") : _("TOTAL ORDER"), - 2);  // PS
		$rep->TextCol(7, 8,	$DisplayTotal, -2);  // PS
