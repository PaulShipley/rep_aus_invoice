...
//			}
			...
			if ($rep->formData['prepaid'])
			{
   			...
   			$result = get_customer_trans_details_aus(ST_SALESINVOICE, $row['trans_no']);  // PS
			$SubTotal = 0;
			while ($myrow2=db_fetch($result))
			{
				...
				$Net = round2($sign * ((1 - $myrow2["discount_percent"]) * $myrow2["unit_price"] * $myrow2["quantity"]),
				   user_price_dec());
				$SubTotal += $Net;
	    		$DisplayPrice = number_format2($myrow2["UnitPriceExTax"],$dec);  // PS
	    		$DisplayGST = number_format2($myrow2["unit_tax"],$dec);  // PS
				...
				if ($Net != 0.0 || !is_service($myrow2['mb_flag']) || !$SysPrefs->no_zero_lines_amount())
				{
					...
					$rep->TextCol($c++, $c,	$DisplayGST, -2);  // PS
    			...
    			if ($myrow['tax_included'])
    			{
    				if ($SysPrefs->alternative_tax_include_on_docs() == 1)
    				{
    					if ($first)
    					{
							...
							$rep->TextCol(7, 8,	number_format2($sign*$tax_item['net_amount'], $dec), -2);  // PS
							$rep->NewLine();
    					}
						...
						$rep->TextCol(3, 7, _("Included") . " " . $tax_type_name . " " . _("Amount"). ": " . $DisplayTax, -2);  // PS
				}
    			else
    			{
					$rep->TextCol(3, 6, $tax_type_name, -2);
			...
			if ($rep->formData['prepaid'])
			{
				...
				$rep->TextCol(7, 8, number_format2($myrow['prep_amount'], $dec), -2);  // PS
			}
